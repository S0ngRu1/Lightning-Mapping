...
threshold_factor = 5;      % find_pulses_advanced 的阈值因子
...
% --- 文件写入准备 ---
filename = 'result_yld_pulse_emd_filtered_10_100_'  + string(pulses_per_group) + '.txt';
...
for j = 1:num_total_blocks
    ...
    [filtered_signal1,~] = extract_band_emd(ch1, fs, 10e6, 100e6,false);
    [filtered_signal2,~] = extract_band_emd(ch2, fs, 10e6, 100e6,false);
    [filtered_signal3,~] = extract_band_emd(ch3, fs, 10e6, 100e6,false);
