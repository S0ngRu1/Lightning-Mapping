...
yld_result_path = 'result_yld_window1024_256_阈值_5std_3.6-4——30——80.txt';
start_signal_loc = 3.65e8;
mapping_start_signal_loc = 3.65e8;
end_signal_loc = 4e8;
step = 127200;%每点0.00393224ns误差，每1272个点误差5ns（1个采样点），每隔127200个点向左平移100个点
% step = 149000;
% 引入两个站的位置关系
...
for j = 1:numel(all_start_signal_loc)-1
    ...
    chj_ch3 =read_signal('..\\2024 822 85933.651462CH3.dat',signal_length,start_read_loc_yld+ 34236722-(j-1)*100 +215/5);%0.77*c
...
%         % 计算三个通道的包络
%         env1 = abs(hilbert(chj_match_signal1));
%         env2 = abs(hilbert(chj_match_signal2));
%         env3 = abs(hilbert(chj_match_signal3));
%         % 合成信号
...
%         % 寻找峰值
%         [peaks, locs] = findpeaks(combined_signal, 'MinPeakHeight', dynamic_threshold, 'MinPeakDistance', 64);
...
%         % 寻找峰值
        ...
        for pi = 1:num_peaks
            ...
            if chj_start_loc == 0 && chj_Rcorr < 0.55 && chj_t123 > 1
                continue
            end
        ...
        match_results = [match_results; struct('yld_start_loc', yld_start_loc(i), 'chj_loc', sub_chj_locs(max_R_gcc_index)+ start_read_loc_yld + 34236722 -(j-1)*100 +start_read_loc_chj-match_signal_length+1,'chj_azimuth',sub_chj_azimuth(max_R_gcc_index),'chj_elevation',sub_chj_elevation(max_R_gcc_index), 'r_gccs', max_R_gcc, 'dlta',dltas(max_R_gcc_index))];
    end
    close(h);
    % 创建文件名
