...
for j = 1:numel(all_start_signal_loc)-1
    ...
    for i =1 :numel(yld_start_loc)
        ...
        if yld_Rcorr(i) < 0.6 && abs(yld_t123(i)) > 1 && yld_elevation(i)  > 80 && yld_elevation(i)  < 10
            continue
        end
        % 转换绝对位置到相对位置
        ...
        num_candidates = numel(locs);
        candidate_R_gccs = zeros(num_candidates, 1);
        candidate_t_gccs = zeros(num_candidates, 1);
        ...
        for pi = 1:num_candidates
            ...
            candidate_R_gccs(pi) = max(r_gcc);
            candidate_t_gccs(pi) = cal_tau(r_gcc, lags_gcc');
        end
        [max_R_gcc, best_candidate_idx] = max(candidate_R_gccs);
        if isempty(max_R_gcc) || max_R_gcc < 0.1
            continue;
        end

        best_loc = locs(best_candidate_idx);
        best_t_gcc = candidate_t_gccs(best_candidate_idx);
        chj_start_idx = best_loc - (chj_signal_length / 2) + 1 - best_t_gcc  ;
        chj_end_idx   = best_loc + (chj_signal_length / 2) - best_t_gcc;

        % 边界检查
        if chj_end_idx > length(chj_match_signal1) || chj_start_idx < 1
            continue;
        end
