...
% 2. 互相关
[xc_1, lags_1] = xcorr(sig_ref_up, sig_del_up);
...
% 1. 原始互相关
[xc_raw, lags_raw] = xcorr(sig_ref, sig_delayed);
...
subplot(2, 2, [1, 2]); % 占据第一行
t_up = linspace(min(t), max(t), length(sig_ref_up));

% 画原始信号（用火柴杆图 stem 表示离散采样）
...
xlim([min(t)*1e6, max(t)*1e6]); % 对齐时间轴
