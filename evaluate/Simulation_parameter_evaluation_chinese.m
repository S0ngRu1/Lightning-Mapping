...
num_trials = 300;           
SNR_dB_range = 0:4:20; % Adjusted to match the uploaded image range roughly
f1 = 20e6; f2 = 80e6;
...
%% ========================================================================
%  Experiment 2: Adaptive Windowing (Window Size Impact) [MODIFIED]
% ========================================================================
fprintf('Running Exp 2: Window Size Impact Analysis...\n');

% 1. 设置实验参数
% 定义不同的信号长度 (代表信号密度/持续时间)
scenarios = [400, 900, 1900, 3900];
scenario_names = {'High Density ', 'Medium Density ', ...
                  'Low Density ', 'Sparse Signal '};
...
% 固定参数
fixed_upsample = 10;
fixed_SNR_dB = 10; % 固定在 15dB 进行观察，以凸显窗口匹配的影响

% 2. 运行仿真
% 结果矩阵: 行=不同信号长度, 列=不同窗口长度
res_exp2_window = zeros(length(scenarios), length(test_wins)); 

for s = 1:length(scenarios)
    curr_sig_len = scenarios(s);
    fprintf('  -> Simulating Signal Length L=%d...\n', curr_sig_len);
    
    for w = 1:length(test_wins)
        curr_win = test_wins(w);
        
        % 注意：这里传入的是单个 fixed_SNR_dB，run_core_sim 会返回单个 STD 值
        sim_val = run_core_sim(fixed_SNR_dB, num_trials, curr_win, ...
                               fixed_upsample, 0, ...
                               curr_sig_len, fs, N_total, true_delay_samples, b_filt, a_filt);
        res_exp2_window(s, w) = sim_val;
    end
end
...
% --- Figure 2: Window Length vs. Signal Density ---
figure('Units', 'inches', 'Position', [8, 1, 7, 5], 'Color', 'w'); % 宽一些的单幅图
ax2 = gca;
hold on; box on; grid on;

% 定义 JGR 风格颜色 (使用 distinct colors)
colors_exp2 = [
    0.8500 0.3250 0.0980; % Orange
...
% 绘制曲线
for s = 1:length(scenarios)
    plot(test_wins, res_exp2_window(s, :), ...
        'LineStyle', '-', ...
        'LineWidth', 1.5, ...
        'Color', colors_exp2(s, :), ...
        'Marker', markers{s}, ...
        'MarkerFaceColor', colors_exp2(s, :), ...
        'MarkerSize', 7);
end

% 坐标轴与标签设置
set(gca, 'YScale', 'log', 'FontName', 'Arial', 'FontSize', 10);
xlabel('Window Length (samples)', 'FontName', 'Arial', 'FontSize', 11, 'FontWeight', 'bold');
ylabel('Time Delay STD (s)', 'FontName', 'Arial', 'FontSize', 11, 'FontWeight', 'bold');
title(['Effect of Window Length on Different Signal Densities (SNR = ' num2str(fixed_SNR_dB) ' dB)'], ...
      'FontName', 'Arial', 'FontSize', 12);

% 设置坐标范围美化
xlim([min(test_wins)-100, max(test_wins)+100]);
ylim([1e-10, 1e-6]); % 根据数据自动调整，或者手动锁定

% 图例设置
legend(scenario_names, 'Location', 'northeast', 'Box', 'off', 'FontName', 'Arial', 'FontSize', 10);

% 添加网格细节
grid on; 
set(gca, 'GridLineStyle', ':', 'GridAlpha', 0.6);

fprintf('Figure 2 generated: Window Size Analysis.\n');
