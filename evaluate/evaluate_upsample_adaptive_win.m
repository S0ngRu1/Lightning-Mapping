...
% --- 文件路径 ---
% 1. 第一列：无上采样自适应 (New)
file_no_upsample = '..\2024\2d\results\result_yld_ADAPTIVE_parabolic.txt';

% 2. 第二列：自适应上采样 (Original Adaptive)
file_upsample = '..\2024\2d\results\result_yld_3.65e8_5.6e8_window_ADAPTIVE_1e4_factor3.txt';
...
% Col 1: 无上采样自适应 (使用与自适应相同的筛选逻辑: t123=1.0, Rcorr基准=0.1)
data_no_up = read_and_filter(file_no_upsample, Start_loc_Base, 1.0, 0.2, T_us_max);

% Col 2: 自适应上采样 (原本的自适应结果)
data_up = read_and_filter(file_upsample, Start_loc_Base, 1.0, 0.1, T_us_max);
...
% 定义列标题
col_titles = {'Adaptive (No Upsample)', 'Adaptive (Upsample)'};
...
% Col 1: No Upsample
nexttile;
plot_time_scatter(data_no_up, 'Azimuth', pt_size, alpha_val, T_us_max);
ylabel('Azimuth (°)'); title('Azimuth vs Time');
ylim([125, 170]);

% Col 2: Upsample
nexttile;
plot_time_scatter(data_up, 'Azimuth', pt_size, alpha_val, T_us_max);
...
% Col 1: No Upsample
nexttile;
plot_time_scatter(data_no_up, 'Elevation', pt_size, alpha_val, T_us_max);
ylabel('Elevation (°)'); title('Elevation vs Time');
ylim([5, 45]);

% Col 2: Upsample
nexttile;
plot_time_scatter(data_up, 'Elevation', pt_size, alpha_val, T_us_max);
...
% Col 1: No Upsample
nexttile;
plot_az_el(data_no_up, pt_size, alpha_val);
...
% Col 2: Upsample
nexttile;
plot_az_el(data_up, pt_size, alpha_val);
xlabel('Azimuth (°)'); 
title('El vs Az');
xlim([125, 170]);
ylim([5, 45]);
...
fprintf('绘图完成。\n点数统计:\n 无上采样(No Up): %d\n 有上采样(Up):    %d\n', ...
    height(data_no_up), height(data_up));
