...
for j = 1:numel(all_start_signal_loc)-1
    ...
    [yld_start_loc, yld_azimuth, yld_elevation, yld_Rcorr, yld_t123] = read_result(yld_result_path,start_read_loc_yld, ...
        end_read_loc_yld);
    yld_ch1 =read_signal('..\\20240822165932.6610CH1.dat',signal_length,start_read_loc_yld);
    chj_ch1 =read_signal('..\\2024 822 85933.651462CH1.dat',signal_length,start_read_loc_yld+ ...
        34151156 - offsets_init-(j-1)*100);
    chj_ch2 =read_signal('..\\2024 822 85933.651462CH2.dat',signal_length,start_read_loc_yld+ ...
        34151156 - offsets_init-(j-1)*100);
    chj_ch3 =read_signal('..\\2024 822 85933.651462CH3.dat',signal_length,start_read_loc_yld+ ...
        34151156 - offsets_init-(j-1)*100 +215/5);
    ...
    S_results = [];
    match_results = struct('yld_start_loc', {}, 'chj_loc', {}, 'chj_azimuth',{},'chj_elevation',{},'chj_Rcorr',{}, ...
        'chj_t123',{},'r_gccs', {},'dlta',{});
    ...
    for i =1 :numel(yld_start_loc)
        ...
        sub_chj_Rcorr = [];
        sub_chj_t123 = [];
        waitbar(i/numel(yld_start_loc), h, sprintf('位置：%d -- %d ；进度： %.2f%%', ...
            start_read_loc_yld, end_read_loc_yld, i/numel(yld_start_loc)*100));
        ...
        chj_match_signal1 = filtered_chj_signal1(start_read_loc_chj-match_signal_length+1: ...
            start_read_loc_chj+match_signal_length+chj_signal_length);
        chj_match_signal2 = filtered_chj_signal2(start_read_loc_chj-match_signal_length+1: ...
            start_read_loc_chj+match_signal_length+chj_signal_length);
        chj_match_signal3 = filtered_chj_signal3(start_read_loc_chj-match_signal_length+1: ...
            start_read_loc_chj+match_signal_length+chj_signal_length);
        ...
        for pi = 1:num_peaks
            ...
            processed_chj_signal2 = real(windowsignal(detrend(processed_chj_signal2)));
            processed_chj_signal3 = real(windowsignal(detrend(processed_chj_signal3)));
            [chj_start_loc, chj_azimuth, chj_elevation, chj_Rcorr, chj_t123] = get_2d_result_single_window( ...
                start_read_loc_chj,processed_chj_signal1,processed_chj_signal2,processed_chj_signal3,'chj');
            ...
            if ~isempty(sub_S)
                ...
                if dlta <= W
                    ...
                    sub_chj_Rcorr = [sub_chj_Rcorr; chj_Rcorr];
                    sub_chj_t123 = [sub_chj_t123; chj_t123];
                end
            end
        end
        ...
        match_results = [match_results; struct('yld_start_loc', yld_start_loc(i), ...
            ...
            'chj_Rcorr',sub_chj_Rcorr(max_R_gcc_index), ...
            'chj_t123',sub_chj_t123(max_R_gcc_index), ...
            'r_gccs', max_R_gcc, ...
