...
% 读取信号数据
ch1 = read_signal('..\\20230718175104.9180CH1.dat', signal_length1,r_loction1);
ch2 = read_signal('..\\20230718175104.9180CH2.dat', signal_length1,r_loction1);
ch3 = read_signal('..\\20230718175104.9180CH3.dat', signal_length1,r_loction1);
...
signal_length2 = 5e7;
r_loction2 = 512;
noise = read_signal('..\\20230718175104.9180CH1.dat', signal_length2,r_loction2);
% x = downsample(noise,50);
% plot(x)

noise_mean =0.05 *  mean(noise);
% noise_std = std(noise);
threshold = noise_mean;

% 打开一个文本文件用于写入运行结果
fileID = fopen('result6_threshold3_4-6e8.txt', 'w');
fprintf(fileID, '%-13s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s\n', ...
    'Start_loc','peak','t12', 't13', 't23', 'cos_alpha_opt', 'cos_beta_opt','Azimuth', 'Elevation', 'Rcorr', 't123');

% 寻找峰值
[peaks, locs] = findpeaks(smoothed_signal1, 'MinPeakHeight', threshold, 'MinPeakDistance',window_length/8);
