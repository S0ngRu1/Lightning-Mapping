...
noisy_signal = normalize(chj3);  % 原始信号已经包含噪声
%% 不同滤波器尝试

% % 1. 带通滤波
filtered_signal1 = normalize(filter_bp(chj3, 20e6 ,80e6 ,8));
...
% SURE阈值结果
subplot(5,1,2);
plot(t*1e6, denoised_signal1, 'r', 'LineWidth', 0.5);
title('SURE阈值去噪结果');
xlabel('时间 (μs)');
ylabel('幅度');
grid on;
...
% 自适应阈值结果
subplot(5,1,3);
plot(t*1e6, denoised_signal2, 'g', 'LineWidth', 0.5);
title('自适应阈值去噪结果');
xlabel('时间 (μs)');
ylabel('幅度');
grid on;
...
% 相位保持结果
subplot(5,1,4);
plot(t*1e6, denoised_signal3, 'm', 'LineWidth', 0.5);
title('相位保持去噪结果');
xlabel('时间 (μs)');
ylabel('幅度');
grid on;
...
% 标准小波去噪结果
subplot(5,1,5);
plot(t*1e6, denoised_signal4, 'k', 'LineWidth', 0.5);
title('标准小波去噪结果');
xlabel('时间 (μs)');
ylabel('幅度');
grid on;
