...
chj_signal_start_loc = yld_signal_start_loc-7584077;
...
subsignal_start = 1:subsignal_length:length(filtered_chj_signal);
for subi = 1:numel(subsignal_start)
    subsignal1 = filtered_chj_signal(subsignal_start(subi):subsignal_start(subi)+subsignal_length-1);
...
for pi = 1:num_peaks
    idx = all_locs(pi);
    % 确保峰值不超出信号范围
    if idx - (yld_signal_length / 2 - 1) <= 0 || idx + (yld_signal_length / 2) > length(filtered_chj_signal)
        continue;
    end
    % 截取窗口信号
    signal1 = filtered_chj_signal(idx-(yld_signal_length/2-1):idx+(yld_signal_length/2));
    % 去直流分量并应用窗函数
...
for i = 1:10
    start_read_loc_chj_top10(i) = chj_signal_start_loc + all_locs(top10_r_gcc_idxs(i)) - 512 + all_t_gccs(top10_r_gcc_idxs(i));
end
end
