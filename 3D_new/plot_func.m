...
signal_length = 1024;
r_loction_yld = 369480077;
r_loction_chj = 403716346;
ch_yld = read_signal('..\\20240822165932.6610CH1.dat',signal_length,r_loction_yld-512);
ch_chj = read_signal('..\\2024 822 85933.651462CH1.dat',signal_length,r_loction_chj-512);
% filtered_yld = filter_bp(ch_yld,20e6,80e6,5);
% filtered_chj = filter_bp(ch_chj,20e6,80e6,5);
ch_yld = normalize(ch_yld);
ch_chj = normalize(ch_chj);
ch_yld = filter_xb(ch_yld);
ch_chj = filter_xb(ch_chj);
subplot(2,1,1);plot(ch_yld);title('yld');xlabel('采样点数');ylabel('幅值');
subplot(2,1,2);plot(ch_chj);title('chj');xlabel('采样点数');ylabel('幅值');
