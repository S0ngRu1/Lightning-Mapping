function matched_peaks_x = match_peaks(peaks1,peaks2,peaks3)
    ...
    for i = 1:numel(peaks1)
        curr_peak1 = peaks1(i);
        % 检查peaks2和peaks3中是否存在与peaks1对应的峰值且x值的差不大于1
        idx_peak2 = find(abs(peaks2 - curr_peak1) <= 1);  % 获取peaks2中匹配峰值的索引
        idx_peak3 = find(abs(peaks3 - curr_peak1) <= 1);  % 获取peaks3中匹配峰值的索引
        % 检查是否找到了匹配的峰值
