...
logicalIndex = abs(result1.t123) < 1 & abs(result1.Rcorr) > 0.3 &  result1.Start_loc > 5.4e8 & result1.Start_loc < 6e8 ;
filteredTable1 = result1(logicalIndex, :);
Start_loc = filteredTable1.Start_loc;
colorValues = (Start_loc - 3e8) / 2e8;
figure;
scatter(filteredTable1.Azimuth,filteredTable1.Elevation, 3, colorValues, 'filled');
...
%自己的结果
logicalIndex = abs(result2.t123) < 1 & abs(result2.Rcorr) > 0.3 &  result2.Start_loc > 4e8 & result2.Start_loc < 6e8;
...
colorValues = (Start_loc - 3e8) / 2e8;
figure;
scatter(filteredTable2.Azimuth,filteredTable2.Elevation, 3, colorValues, 'filled');
title('Azimuth vs Elevation');
xlabel('Azimuth');
xlim([120, 360]);
xticks(120:20:360);
...
% dir('E:\2023data\find_peaks\')
Untitled=importdata('E:\2023data\find_peaks\2023_result.txt');
read_data=Untitled.data;
logicalIndex = abs(read_data(:,11)) < 1 & abs(read_data(:,10)) > 0.3 & read_data(:,1) > 500000000 & read_data(:,1) < 600000000 ;
filteredTable3 = read_data(logicalIndex, :);
Start_loc = filteredTable3(:,1);
colorValues = (Start_loc - 3e8) / 2e8;
figure;
scatter(filteredTable3(:,8),filteredTable3(:,9), 3, colorValues, 'filled');
title('Azimuth vs Elevation');
xlabel('Azimuth');
xlim([0, 360]);
xticks(0:40:360);
