...
% 进行带通滤波
filtered_signal1 = filter_bp(ch1, 20e6, 80e6, 14);
filtered_signal2 = filter_bp(ch2, 20e6, 80e6, 14);
filtered_signal3 = filter_bp(ch3, 20e6, 80e6, 14);
% 平滑信号以减少噪声
...
% 打开文件用于写入
fileID = fopen('result_5.4-6.txt', 'w');
...
for pi = 1:num_peaks
    ...
    % 截取窗口信号
    signal1 = smoothed_signal1(idx-(windows_length/2-1):idx+(windows_length/2));
    signal2 = smoothed_signal2(idx-(windows_length/2-1):idx+(windows_length/2));
    signal3 = smoothed_signal3(idx-(windows_length/2-1):idx+(windows_length/2));

    % 去直流分量并应用窗函数
     ch1_n = detrend(signal1);
     ch2_n = detrend(signal2);
     ch3_n = detrend(signal3);
     ch1_new = windowsignal(ch1_n);
     ch2_new = windowsignal(ch2_n);
     ch3_new = windowsignal(ch3_n);

     % 上采样
     ch1_upsp = upsampling(ch1_new, upsampling_factor)';
     ch2_upsp = upsampling(ch2_new, upsampling_factor)';
     ch3_upsp = upsampling(ch3_new, upsampling_factor)';
    ...
    % 计算时间延迟
    t12 = cal_tau(r12_gcc, lags12_gcc') * 0.1;
    t13 = cal_tau(r13_gcc, lags13_gcc') * 0.1;
    t23 = cal_tau(r23_gcc, lags23_gcc') * 0.1;
    % 计算初始角度
