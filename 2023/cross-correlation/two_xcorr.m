...
% 打开一个文本文件用于写入运行结果
fileID = fopen('result1_kalman0.5_xcorr2.txt', 'w');
fprintf(fileID, 'window_length-1424_Window-1024_Iteration-1024_kalman-0.5_xcorr2\n');
fprintf(fileID, '%-13s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s%-15s\n', ...
    'Start_loc','peak','t12', 't13', 't23', 'cos_alpha_opt', 'cos_beta_opt','Azimuth', 'Elevation', 'Rcorr', 't123');
%寻找信号1的所有满足条件的峰值
peaks = find_peaks(filtered_signal1,88);
t = 1;
%遍历所有峰值
for  pi = 1:numel(peaks)
    ...
    signal3 = filtered_signal3(idx-(window_length/2-1):idx+(window_length/2));
        ...
        ch2_newwin = shifted_ch1(35600-(window/2-1):35600+(window/2));
        ch3_newwin = shifted_ch1(35600-(window/2-1):35600+(window/2));

%         信号上采样
%         ch1_upsp = upsampling(ch1_newwin,50)';
%         ch2_upsp = upsampling(ch2_newwin,50)';
%         ch3_upsp = upsampling(ch3_newwin,50)';

        [r12_gcc2,lags12_gcc2] = xcorr(ch1_newwin(:,1),ch2_newwin(:,1),'normalized');
        [r13_gcc2,lags13_gcc2] = xcorr(ch1_newwin(:,1),ch3_newwin(:,1),'normalized');
        [r23_gcc2,lags23_gcc2] = xcorr(ch2_newwin(:,1),ch3_newwin(:,1),'normalized');
        ...
        fprintf(fileID, '%-13d%-15d%-15.6f%-15.6f%-15.6f%-15.6f%-15.6f%-15.6f%-15.6f%-15.6f%-15.6f\n', ...
            440100000+idx,513, t12, t13, t23, cos_alpha_opt, cos_beta_opt, Az_deg, El_deg, Rcorr,t123);
t = pi;
end
% 关闭文件
fclose(fileID);
