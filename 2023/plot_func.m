...
colormap('jet'); 
h = colorbar;
ylabel(h, '归一化发展过程', 'FontSize', 11, 'Color', 'k'); % 如果用Rcorr，这里改写为 '相关系数 Rcorr'
set(h, 'Color', [0.2 0.2 0.2]); 
caxis([0, 1]); % 如果用Rcorr，建议改为 [0.3, 1]

% --- 6. 网格和辅助线 ---
grid on;
set(gca, 'GridLineStyle', '--', 'GridAlpha', 0.3, 'Box', 'on');

% 【可选】添加箭头标注反冲或梯级 (如果有的话)
% annotation('arrow', [0.5 0.6], [0.5 0.6]); 

fprintf('绘图完成！显示了 %d 个辐射源点。\n', height(filteredTable1));


%%
filename = 'results\20230718175104_result_yld_3e8_6e8_window_1024_256_阈值4倍标准差_去零飘_20_80_hann_with_error.txt';
% ""
fs = 200e6; % 采样率 
Start_loc_Base = 3e8+9e7+9000000; % 基准值  
if ~isfile(filename), error('文件不存在'); end
