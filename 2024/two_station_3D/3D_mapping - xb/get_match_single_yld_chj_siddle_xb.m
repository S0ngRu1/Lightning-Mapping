function [start_read_loc_chj, r_gccs] = get_match_single_yld_chj_siddle_xb(yld_signal_start_loc, skip_large_window)
    % 定义逐步缩小的窗口长度，依次进行粗匹配到细匹配
    % 小波去噪参数
    level = 4;          % 分解层数
    wavelet = 'db4';    % 小波基 
    window_lengths = [2e5, 8e4, 2e4, 6000, 1024];
    ...
    for i = 1:length(window_lengths)
        ...
        filtered_yld_signal = filter_xb(yld_signal);
...
%             chj_length = current_window_length * 2;
        ...
        filtered_chj_signal = filter_xb(chj_signal);
        ...
        for subi = 1:numel(subsignal_starts)
            ...
            subsignal_chj = filtered_chj_signal(subsignal_starts(subi) : subsignal_starts(subi) + yld_signal_length - 1);
 ...
 %             filtered_chj_signal = waveletDenoiseAdaptive(subsignal_chj, level, wavelet);
  %             processed_chj_signal = real(windowsignal(detrend(filtered_chj_signal)));
            ...
            [r_gcc, lags_gcc] = xcorr( subsignal_chj,filtered_yld_signal,'normalized');
            R_gcc = max(r_gcc);
