%% ========================================================================
%  主程序: 脉冲级特征匹配定位算法+(基于高精度计数的)四档自适应窗口
%  ========================================================================
...
for j = 1:numel(all_start_signal_loc)-1
    ...
    scout_pulse_catalog = find_pulses_advanced(filtered_yld_signal1, 3.545, fs, 3);
    pulse_count_in_chunk = numel(scout_pulse_catalog);
    % 调用决策函数，获得当前信号块应该使用的窗口长度
    dynamic_snippet_len = get_adaptive_window_length_4tier(pulse_count_in_chunk, signal_length);
    filtered_yld_signal2 = filter_bp(yld_ch2,30e6,80e6,5);
    filtered_yld_signal3 = filter_bp(yld_ch3,30e6,80e6,5);
    %% --- 1. 独立脉冲编目  ---
    % 创建脉冲目录
    yld_catalog = create_pulse_catalog(filtered_yld_signal1, filtered_yld_signal2, filtered_yld_signal3, fs, threshold_yld, dynamic_snippet_len, upsampling_factor, msw_length);
    % 检查目录是否为空
    ...
    for i =1 :num_yld_pulses
        ...
        chj_catalog = create_pulse_catalog(filtered_chj_signal1, filtered_chj_signal2, filtered_chj_signal3, fs, threshold_chj, dynamic_snippet_len, upsampling_factor, msw_length);
...
%                     我们还可以记录损失
                    match_info_dtoa = struct(...
                        'yld_start_loc', start_signal_loc + current_yld_pulse.loc - dynamic_snippet_len/2 + 1, ...
                        'chj_loc', matched_chj_pulse.loc + start_signal_loc + 34226222 - dynamic_snippet_len/2 +1, ...
                        'chj_azimuth', chj_az, ...
                        ...
                        'cost', match_cost,  ...
                        'dlta_T',dlta_T, ...
                        'dynamic_snippet_len',dynamic_snippet_len ...
                        );
