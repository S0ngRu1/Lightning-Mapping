...
chj_sit = [2003.7972, -7844.7836, -27]; % 观测站2 (B2) 的位置 (x,y,z)

% 定义一个已知的目标三维点
target_true_pos = [15000, -30000, 3000]; % 例如 (x,y,z)
...
% 计算从零点位置指向当前定位结果点的向量
    direction_vector = target_true_pos(:) - yld_sit(:);

    % 调用 cart2sph_standard 函数计算方位角和仰角
    [azimuths, elevations] = cart2sph_standard(direction_vector);
