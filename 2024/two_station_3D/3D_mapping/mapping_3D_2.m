%方法2：先逐步匹配缩小时间差，再根据d/c条件筛选（get_match_single_yld_chj_siddle_2）
...
for i =1 :numel(yld_start_loc)
    ...
    [start_read_loc_chj, r_gccs, result_2e7] = get_match_single_yld_chj_siddle_2(yld_start_loc(i), skip_large);
    if result_2e7 ~= 0
        last_large_match_result = result_2e7;
    end

    % 读取 match_signal_length*2 长度的信号(yld信号窗口左右两边相差<6000点d/c）
...
% 绘制过滤后的数据
figure;
scatter3(filtered_S(:,1), filtered_S(:,2), filtered_S(:,3), 2, 'filled');
xlabel('X');
ylabel('Y');
zlabel('Z');
title('过滤后的所有 S 点的分布');
