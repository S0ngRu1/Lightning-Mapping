...
if strcmp(window,'hann')
    % 创建一个改进汉宁窗
    win = exp_hanning(processing_window_len, 5);
    win = win(:); % 确保是列向量
...
if isempty(win)
        windowed_ch1 = conv_window(detrend(ch1), template_norm);
        windowed_ch2 = conv_window(detrend(ch2), template_norm);
        windowed_ch3 = conv_window(detrend(ch3), template_norm);
    else
...
file_name = 'result_yld_3.8e8_4e8_window_512_128_去零飘——先加窗_'+string(fp_start/1e6)+'_'+string(fp_end/1e6)+'_'+ window +'.txt';
% 打开一个文本文件用于写入运行结果
